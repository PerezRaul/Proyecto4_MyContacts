    ...
    <body cz-shorcut-listen="true">
        ...
        ?>
        	<div id="cont">
                ...
                <?php
                    include('conexion.php');
                    $sql_mod_usu = "SELECT * FROM tbl_login WHERE log_id=$_REQUEST[usu_id]";
                    $datos_mod_usu = mysqli_query($con, $sql_mod_usu);

                    if(mysqli_num_rows($datos_mod_usu) > 0){
                        $modificar = mysqli_fetch_array($datos_mod_usu);
                ?>
        					<form name="agregar" action="modificar_usuario.proc.php" method="post">
        						<input type="hidden" name="id" value="<?php echo $modificar['log_id']; ?>">
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-user"></i></span>
                                    <input type="text" class="form-control" name="username" placeholder="Username" maxlength="25" value="<?php echo $modificar['log_username']; ?>" />
                                </div><br />
                                <div class="input-group">
                                    <span class="input-group-addon"><i class="fa fa-at"></i></span>
                                    <input type="text" class="form-control" name="mail" placeholder="E-mail" maxlength="50" value="<?php echo $modificar['log_mail']; ?>" />
                                </div><br />
        						<div>
        				            <?php
        				                if(isset($error)){
        						...
        						<input type="submit" class="btn btn-success" name="crear" value="Crear" /> <?php echo "<a href='eliminar_usuario.php?usu_id=$_SESSION[id]'>";?><input type="button" class="btn btn-danger" name="borrar" value="Eliminar" /></a> <?php echo "<a href='cambiar_pass.php?usu_id=$_SESSION[id]'>Cambiar password</a>"; ?>
        					</form>
    				</article>
    			</section><br />
    			<footer id="foot">
